...
# Debug list at the end

function lobpcg_antiherm(M) # antihermitian part of M
    n = size(M,1)
    @assert size(M) == (n,n)
    s = sum(abs.(M))
    if s<1e-10
...
function lobpcg_assert_antiherm(M,tol)
    h = lobpcg_antiherm(M) # antihermitian part of M
    if h>tol
        println("Antiherm problem ",h)
        @assert h<tol
    end
end
...
function LOBPCG(A, X, D, B=I, precon=((Y, X, R)->R), tol=1e-10, maxiter=100;
    ...
    # println("N=",N," M=",M)
    N >= 3M || @warn "Your problem is too small, and LOBPCG might fail; use a full diagonalization instead"
...
# Finds the first l eigenmodes of the action H. Full diagonalization if the dimension is too low
function solve_lobpcg(H,l,k_list_squared;maxiter=100,tol=1e-5,X0=-1,full_diag=false,prints=true)
    # H can be a matrix
    L = H
    if H isa Matrix
        L = x -> H*x
    ...
    # print(L(ones(ComplexF64,n)))
    @assert n >= l
    @assert l == size(X0,2)
    λs = []; cv = true; Xf = -1
    dt = -1
    if !full_diag && 3*l < n
        if prints println("LOBPCG : n is ",n," X0 has size ",size(X0)) end
        t0 = time()
        res = lobpcg_hyper(L,X0,k_list_squared; maxiter=maxiter, tol=tol)
        dt = time()-t0
        if prints println("LOBPCG Converged ? ",res.converged," | Iterations= ",res.iterations) end
        λs = res.λ
        cv = res.converged
        Xf = res.X
        # φs = [Xf[:,i] for i in 1:size(Xf,2)]
    else
        matH = (H isa Matrix) ? H : linmap2mat(L,n)
        lobpcg_assert_antiherm(matH,1e-10)
        ...
        λs = sol.values
        Xf = sol.vectors
        # φs = [Xf[:,i]/sqrt(sum(abs2.(Xf[:,i]))) for i=1:l]
    end
    if prints println("LOBPCG took ",round(dt,digits=5)," sec") end
    (λs,cv,Xf) # update : ne rend plus φs
end

# Transforms an output X into a list of eigenvectors
function X_to_φs(X)
    vecs = []
    n, l = size(X,1), size(X,2)
    println("Type of X is ",typeof(X))
    println("Size of X : ",n," × ",l)
    for i=1:l
        v = X[:,i]/sqrt(sum(abs2.(X[:,i])))
        push!(vecs,v)
    end
    vecs
end

# DEBUG : Ortho is failing badly could mean that the preconditionner is not good
